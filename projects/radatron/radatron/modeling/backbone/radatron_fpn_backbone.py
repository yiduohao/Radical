...
def build_radatron_resnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    if cfg.PRETRAINED_BACKBONE_PART == 'bottom_up':
        for i in range(10):
            print("Loading pretrained weights for bottom_up")
            print(cfg.PRETRAINED_BACKBONE_PATH)
        bottom_up_dict = torch.load(cfg.PRETRAINED_BACKBONE_PATH)
        bottom_up.load_state_dict(bottom_up_dict['state_dict'])
    if cfg.MODEL.BACKBONE.FREEZE_AT >= 10:
        bottom_up.call_freeze(cfg.MODEL.BACKBONE.FREEZE_AT)
    ...
    if cfg.PRETRAINED_BACKBONE_PART == 'backbone':
        for i in range(10):
            print("Loading pretrained weights for backbone")
            print(cfg.PRETRAINED_BACKBONE_PATH)
        backbone_dict = torch.load(cfg.PRETRAINED_BACKBONE_PATH)
        backbone.load_state_dict(backbone_dict['state_dict'])
    return backbone
