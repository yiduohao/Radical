...
def register_radatron_ssl(cfg):
    train_set = RadatronDataset(cfg=cfg, val=False, ssl=cfg.DATALOADING.SSL)
    # test_set = RadatronDataset(cfg=cfg, val=True, ssl=cfg.DATALOADING.SSL)

    DatasetCatalog.register("radar_train", lambda d="train": train_set.get_dataset_dict_ssl())
    MetadataCatalog.get("radar_train").set(thing_classes=cfg.DATAPATHS.CATEGORIES)
